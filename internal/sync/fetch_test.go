...
func TestFetchNewMessages_ArchivesEverythingAboveLastUID(t *testing.T) {
	...
	stats, err := FetchNewMessages(context.Background(), c, env.accountID, folder, mw, env.w, env.emailsR, env.foldersR, env.attachmentsR, nil, nil, nil, nil)
	if err != nil {
...
func TestFetchNewMessages_SkipsWhenSyncDisabled(t *testing.T) {
	...
	stats, err := FetchNewMessages(context.Background(), nil, env.accountID, folder, mw, env.w, env.emailsR, env.foldersR, env.attachmentsR, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("FetchNewMessages: %v", err)
	}
	if stats.Archived != 0 {
...
func TestFetchNewMessages_StopsOnCancelledContext(t *testing.T) {
	...
	mw := env.newWriter(t, "INBOX")
	stats, err := FetchNewMessages(ctx, c, env.accountID, folder, mw, env.w, env.emailsR, env.foldersR, env.attachmentsR, nil, nil, nil, nil)
...
func TestFetchNewMessages_ReportsProgressPerMessage(t *testing.T) {
	...
	stats, err := FetchNewMessages(context.Background(), c, env.accountID, folder, mw, env.w, env.emailsR, env.foldersR, env.attachmentsR, nil, nil, nil, onProgress)
	if err != nil {
		t.Fatalf("FetchNewMessages: %v", err)
	}
	if stats.Archived != 3 {
